...
class RPiPinIdSetTestCases(unittest.TestCase):
    def setUp(self):
      self.rev_idx_range = range(2); # values 0 and 1 in range
...
class PinIdRevisionUnitTestCases(unittest.TestCase):
    def test_0000_rpi_revision_accept_1_and_2_give_revision_indexes_0_and_1(self):
      pinid.PinId._set_rpi_major_revision(1)
      self.assertEqual( pinid.PinId._get_rpi_major_revision_index(), 0 )
      pinid.PinId._set_rpi_major_revision(2)
      self.assertEqual( pinid.PinId._get_rpi_major_revision_index(), 1 )

    def test_0020_rpi_revision_other_than_1_and_2_rasie_exception(self):
      with self.assertRaises( pinid.PinIdInvalidRevisionError ):
        pinid.PinId._set_rpi_major_revision(0)
      with self.assertRaises( pinid.PinIdInvalidRevisionError ):
        pinid.PinId._set_rpi_major_revision(3)
      with self.assertRaises( pinid.PinIdInvalidRevisionError ):
        pinid.PinId._set_rpi_major_revision(None)
      with self.assertRaises( pinid.PinIdInvalidRevisionError ):
        pinid.PinId._set_rpi_major_revision("crap")
...
class PinIdRPiRev1UnitTestCases(unittest.TestCase):
    def setUp(self):
      pinid.PinId._set_rpi_major_revision(1)
      self.rev_idx = pinid.PinId._get_rpi_major_revision_index()
